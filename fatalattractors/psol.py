...
def sort_nodes_by_priority(g):
    ...
    :rtype:
    """
    # TODO here we used g.nodes and not get_nodes because it yields only the keys
    ...
    return [k[0] for k in sorted(g.nodes.items(), key=lambda x: x[1][1])]
        ...
        for sbis in g.get_predecessors(s):
            if DEBUG_PRINT: print(
                "         Considering predecessor " + str(sbis) + " Is marked ? " + str(
                    ...
                    g.get_node_player(sbis)) + " Priority " + str(g.get_node_priority(sbis)))
...
def psol(g, W1, W2):
    ...
    for node in sort_nodes_by_priority(g):
        ...
        if node in MA:
            ...
            att, complement = attractor(g, MA, g.get_node_priority(node) % 2)

            if g.get_node_priority(node) % 2 == 0:
                W1.extend(att)
            else:
                W2.extend(att)
